...
export default function AuthGate({ children }) {
  ...
  // 订阅 Supabase 会话：登录/登出/初始化都由它驱动，无需手动管理本地 token
  useEffect(() => {
    let unsub;
  ...
  // 本地简易映射（email → displayName），最终以 auth.getUser 返回为准
  function mapLocal(u) {
    return {
      id: u.id,
      email: u.email || '',
  ...
  const doSubmit = async (e) => {
    ...
    if (tab === 'password') {
      ...
      setBusy(true);
      try { await login(email, password); /* 会话由订阅自动置为已登录 */ }
    ...
    return (
      <div className="min-root">
        <style>{`
          *, *::before, *::after { box-sizing: border-box; }
          ...
          .min-brand-ico {
            ...
            display: flex; align-items: center; justify-content: center;
            box-shadow: 0 6px 18px -6px rgba(17,17,19,0.35);
          }
          ...
          .min-tabs {
            ...
            margin: 28px 0 22px; padding: 4px;
            border-radius: 12px; background: #F4F4F5; border: 1px solid #ECECEF;
          }
          .min-tab {
            flex: 1; appearance: none; border: none; cursor: pointer;
            padding: 10px 0; border-radius: 9px; background: transparent; color: #9CA1AB;
            font: 600 13px/1 Inter, "PingFang SC", system-ui, sans-serif;
          ...
          .min-forget {
            ...
            font: 600 12px/1 Inter, system-ui, sans-serif; color: #7C5CFF;
          ...
          .min-input {
            ...
            border-radius: 11px; border: 1.5px solid #E4E4E7; background: #FCFCFD; color: #111113;
            font: 500 14.5px/1 Inter, "PingFang SC", system-ui, sans-serif; outline: none; transition: all .3s ease;
          ...
          .min-input.min-pw-right { padding: 0 44px 0 44px; }
          .min-toggle-pw {
          ...
          .min-send-link {
            ...
            border-radius: 11px; border: 1.5px solid #E4E4E7; background: #FFFFFF; color: #111113;
            font: 700 14px/1 Inter, "PingFang SC", system-ui, sans-serif; cursor: pointer;
            transition: all .3s ease;
          }
          ...
          .min-error {
            ...
            border-radius: 11px; background: #FEF2F2; border: 1px solid #FECACA; color: #DC2626;
            font: 600 12.5px/1.4 Inter, "PingFang SC", system-ui, sans-serif;
          }

          .min-submit {
            position: relative; width: 100%; height: 48px;
            display: inline-flex; align-items: center; justify-content: center; gap: 8px;
            padding: 0 22px; border: none; cursor: pointer; border-radius: 11px;
            color: #FFFFFF; font: 700 14.5px/1 Inter, "PingFang SC", system-ui, sans-serif; background: #111113;
            transition: all .3s ease;
          }
          .min-submit:hover:not(:disabled) { background: #7C5CFF; }
          .min-submit:active:not(:disabled) { transform: translateY(1px) scale(.99); }
          .min-submit:disabled { opacity: .5; cursor: not-allowed; }
          .min-submit .min-spinner {
            width: 16px; height: 16px; border-radius: 50%;
            border: 2px solid rgba(255,255,255,0.35); border-top-color: #FFFFFF;
            animation: minspin .7s linear infinite;
          }
          @keyframes minspin { to { transform: rotate(360deg); } }

          .min-divider { display: flex; align-items: center; gap: 12px; color: #A1A1AA; font-size: 11.5px; font-weight: 600; letter-spacing: .04em; }
          ...
          .min-register-hint {
            width: 100%; margin: 28px 0 22px; padding: 10px 0;
            border-radius: 12px; background: #F4F4F5; border: 1px solid #ECECEF; color: #52525B;
            font: 600 13px/1 Inter, "PingFang SC", system-ui, sans-serif; text-align: center;
          }

          .min-switch {
            margin-top: 22px; appearance: none; border: none; background: none; padding: 6px 10px; cursor: pointer;
            font: 600 13px/1 Inter, "PingFang SC", system-ui, sans-serif; color: #7C5CFF;
            transition: color .3s ease;
          }
        ...
        <div className="min-card">
          ...
          <h2 className="min-title">{mode === 'register' ? '创建账号' : '欢迎登录'}</h2>
          <p className="min-sub">{mode === 'register' ? '注册后即可同步你的全部个人工具数据' : '登录后可同步你的全部个人工具数据'}</p>

          {mode === 'login' ? (
            <div className="min-tabs">
              <button type="button" className={'min-tab ' + (tab === 'password' ? 'on' : '')} onClick={() => switchTab('password')}>
                邮箱密码
              </button>
              <button type="button" className={'min-tab ' + (tab === 'link' ? 'on' : '')} onClick={() => switchTab('link')}>
                邮箱登录链接
              </button>
            </div>
          ) : (
            <div className="min-register-hint">创建账号 · 使用邮箱和密码注册</div>
          )}
          ...
          <form onSubmit={doSubmit} className="min-form">
            <div className="min-field">
              ...
              <div className="min-input-wrap">
                ...
                <input
                  ...
                  placeholder="you@example.com" className="min-input" autoFocus
                />
              </div>
            </div>

            {/* 邮箱密码 / 注册密码 */}
            {mode === 'register' || tab === 'password' ? (
              <>
                <div className="min-field">
                  ...
                  <div className="min-input-wrap">
                    ...
                    <input
                      ...
                      placeholder="请输入密码" className="min-input min-pw-right"
                      autoComplete={mode === 'login' ? 'current-password' : 'new-password'}
                    />
                    <button type="button" className="min-toggle-pw" onClick={() => setShowPw((v) => !v)} tabIndex={-1}
            ...
            {err && (
              <div className="min-error"><ShieldCheck size={14} strokeWidth={1.8} /><span>{err}</span></div>
            )}
            {msg && <span className="min-code-msg">{msg}</span>}

            {mode !== 'login' || tab === 'password' ? (
              <button type="submit" disabled={busy || !email || (mode === 'register' ? !password || !confirm : !password)} className="min-submit">
                {busy ? (<><span className="min-spinner" /><span>{mode === 'register' ? '注册中…' : '登录中…'}</span></>)
                  : (<><span>{mode === 'register' ? '注册' : '登录'}</span><ArrowRight size={17} strokeWidth={2.2} /></>)}
              </button>
            ) : (
          ...
          <button type="button" onClick={() => switchMode(mode === 'login' ? 'register' : 'login')} className="min-switch">
