...
export default function CanvasFrame() {
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            <Play className="h-7 w-7 text-[var(--text-1)]" strokeWidth={1.5} fill="currentColor" />
          </div>

          {/* 标题 */}
          <h2 className="text-xl font-semibold tracking-tight text-[var(--text-1)] mb-2">视频工作流</h2>

          {/* 描述 */}
          <p className="text-[13px] text-[var(--text-2)] text-center max-w-xs leading-relaxed mb-8">
            本地画布服务 · AI 视频生成工作台
          </p>
          ...
          >
            <span>点击进入</span>
            <ArrowRight className="h-3.5 w-3.5 text-[var(--text-3)]" strokeWidth={1.8} />
          </div>
        </div>
  ...
  if (!checked) {
    return (
      <div className="glass flex flex-col items-center justify-center py-24 animate-fade-in" style={{ minHeight: 420 }}>
        ...
        <div className="text-[13px] text-[var(--text-2)]">正在连接本地画布服务…</div>
        <div className="mt-1 text-[11px] text-[var(--text-3)]">服务启动中，请稍候</div>
      </div>
    );
  }
  ...
  if (!status.ready || !status.url) {
    return (
      <div className="glass flex flex-col items-center justify-center py-20 animate-fade-in" style={{ minHeight: 420 }}>
        <div className="mb-3 inline-flex h-12 w-12 items-center justify-center rounded-xl bg-[var(--bg-2)]">
          <PanelsTopLeft className="h-6 w-6 text-[var(--text-3)]" strokeWidth={1.6} />
        </div>
        <div className="text-[13px] font-medium text-[var(--text-2)]">本地画布服务未就绪</div>
        <div className="mt-1 text-[11px] text-[var(--text-3)] max-w-sm text-center">
  ...
  if (loadFailed && !frameLoaded) {
    return (
      <div className="glass flex flex-col items-center justify-center py-20 animate-fade-in" style={{ minHeight: 420 }}>
        ...
        <div className="text-[13px] font-medium text-[var(--text-2)]">画布页面加载异常</div>
        <div className="mt-1 text-[11px] text-[var(--text-3)] max-w-sm text-center">
  ...
  return (
    ...
    >
      ...
      >
        <div className="h-[3px] w-56 rounded-full overflow-hidden" style={{ background: 'var(--bg-4)' }}>
          <div
            style={{
              width: '42%',
              height: '100%',
        ...
        <div className="mt-4 text-[12px] text-[var(--text-2)]">正在加载画布…</div>
      </div>
    </div>
  );
}