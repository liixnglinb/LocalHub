 ...
 * 规范：
 * - 浮层：大圆角矩形，亮色磨砂毛玻璃半透明叠加 + 轻微模糊，无外发光、无过重阴影
 * - 浮层紧贴触发按钮下方（或上方，自动检测视口空间），禁止居中
 * - 条目垂直排列，无分割线；常态白底深色文字；选中项右侧对勾√
 * - hover：整行浅灰高亮 var(--hover)，文字保持深色
    ...
    <div ref={wrapRef} className={`relative inline-block ${className}`} style={{ width: width || undefined }}>
      ...
      {trigger ? (
        ...
        <button
          ...
          disabled={disabled}
          onClick={() => (open ? setOpen(false) : openMenu())}
          className="flex w-full items-center justify-between gap-2 px-3 py-2 text-[13px] text-[var(--text-1)] transition-colors disabled:opacity-42"
          style={{
            background: 'var(--bg-1)',
        ...
        >
          ...
          <ChevronDown className="h-3.5 w-3.5 shrink-0 text-[var(--text-3)]" strokeWidth={1.8} />
        </button>
      )}
            ...
            return (
              <button
                ...
                className={`flex w-full items-center gap-2 rounded-[8px] px-3 py-[9px] text-left text-[13px] transition-colors ${
                  opt.danger ? 'text-[var(--danger)]' : 'text-[var(--text-1)]'
                }`}
                style={{ background: selected ? 'var(--sel)' : 'transparent', minHeight: 38 }}
                onMouseEnter={(e) => { e.currentTarget.style.background = opt.danger ? 'var(--danger-soft)' : 'var(--hover)'; }}
                onMouseLeave={(e) => { e.currentTarget.style.background = selected ? 'var(--sel)' : 'transparent'; }}
              >
                {/* 锁图标（会员专属） */}
                {opt.lock && <Lock className="h-3.5 w-3.5 shrink-0 text-[var(--text-3)]" strokeWidth={1.8} />}
                <span className="flex-1 truncate">{opt.label}</span>
                {/* 选中对勾 */}
                {selected && <Check className="h-4 w-4 shrink-0 text-[var(--text-1)]" strokeWidth={2} />}
              </button>
            );
          })}
        </div>
