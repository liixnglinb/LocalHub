 ...
 * - 时/分两列垂直滚轮：未选中数字带轻微运动模糊（触控滚轮质感），当前选中值电光蓝高亮 + 发光
 * - 白色无衬线字体；上下箭头微调；点击数字跳转；确定后生效
 */
const HOURS = Array.from({ length: 24 }, (_, i) => i);
const MINUTES = Array.from({ length: 60 }, (_, i) => i);
const ELEC_BLUE = '#22C3D6';      // 电光蓝
          ...
          return (
            <div
              ...
              style={{
                ...
                color: isSel ? '#F2FEFF' : '#FFFFFF',   // 选中：亮青白（接近白，清晰可读）
                opacity,
                filter: `blur(${blur}px)`,
                fontVariantNumeric: 'tabular-nums',
                cursor: 'pointer',
                textShadow: isSel ? `0 0 8px rgba(34,195,214,0.45)` : 'none', // 单层小范围微光，不糊字
  ...
  return (
    <div ref={wrapRef} className="relative inline-block" style={{ width }}>
      ...
      <button
        ...
        className="flex items-center justify-center gap-2 px-3 py-2 text-[14px] text-white/90 transition-all"
        style={{
      ...
      >
        <Clock className="h-3.5 w-3.5" strokeWidth={1.8} style={{ color: ELEC_BLUE }} />
      ...
      {open && pos && (
        ...
        >
          ...
          <div className="mb-2 flex items-center justify-center gap-1.5 text-[10px] uppercase tracking-[0.22em] font-semibold" style={{ color: 'rgba(255,255,255,0.35)' }}>
            <Clock className="h-3 w-3" style={{ color: ELEC_BLUE }} strokeWidth={1.8} />
            选择时间
          </div>
          ...
          <div className="flex items-center justify-center">
            ...
            <span className="text-[17px] font-semibold mx-1" style={{ color: 'rgba(255,255,255,0.35)', fontVariantNumeric: 'tabular-nums' }}>:</span>
