...
const SUBJECT_COLORS = {
  数学: 'bg-blue-500/15 text-blue-300 border-blue-400/30',
  英语: 'bg-emerald-500/15 text-emerald-300 border-emerald-400/30',
  编程: 'bg-cyan-500/15 text-cyan-300 border-cyan-400/30',
  物理: 'bg-orange-500/15 text-orange-300 border-orange-400/30',
  化学: 'bg-rose-500/15 text-rose-300 border-rose-400/30',
  历史: 'bg-amber-500/15 text-amber-300 border-amber-400/30',
...
function MindMapCanvas({
  ...
  const gridStyle = {
    ...
    backgroundImage: 'radial-gradient(circle, rgba(255,255,255,0.14) 1.1px, transparent 1.1px)',
  ...
  return (
    ...
    >
      ...
      <div className="absolute" style={{ left: 0, top: 0, width: 0, height: 0, transform: `translate(${offsetX}px, ${offsetY}px) scale(${zoom})`, transformOrigin: '0 0' }}>
        ...
        <svg style={{ position: 'absolute', left: 0, top: 0, overflow: 'visible', width: 0, height: 0 }}>
          {links.map(({ from, to }, i) => {
            ...
            return <path key={i} d={path} fill="none" stroke="rgba(255,255,255,0.28)" strokeWidth="1.8" strokeLinecap="round" />;
          })}
        </svg>
          ...
          return (
            <div
              ...
              style={{
                ...
                border: `1.5px solid ${isSelected ? color : 'rgba(255,255,255,0.16)'}`,
            ...
            >
              <span className="text-[12.5px] font-medium text-white/90 truncate w-full text-center">{node.text}</span>
              {hasNote && (
                <span className="absolute top-0.5 right-1 text-[9px] rounded px-1 py-px font-semibold"
                  style={{
                    color: noteType === 'python' ? '#7dd3fc' : noteType === 'matlab' ? '#fcd34d' : 'rgba(110,231,183,0.9)',
                    background: noteType === 'python' ? 'rgba(56,189,248,0.15)' : noteType === 'matlab' ? 'rgba(251,191,36,0.15)' : 'rgba(52,211,153,0.12)',
                  }}
                >
              ...
              {mode === 'edit' && (
                <button
                  ...
                  className="absolute -right-2 -top-2 h-5 w-5 rounded-full flex items-center justify-center text-white bg-cyan-500/90 hover:bg-cyan-400 shadow-lg opacity-0 group-hover:opacity-100 transition-opacity"
                  title="添加子节点"
                >
        ...
        >
          ...
          <div
            ...
            style={{
              ...
              borderTop: popup.below ? 'none' : '6px solid rgba(255,255,255,0.5)',
              borderBottom: popup.below ? '6px solid rgba(255,255,255,0.5)' : 'none',
            }}
          />
          {mode === 'edit' && renameMode ? (
            <>
              ...
              <input
                ...
                className="w-full text-[12.5px] px-2.5 py-1.5 rounded-[7px] bg-black/40 border border-white/10 outline-none focus:border-cyan-400/50 text-white"
                autoFocus
                placeholder="输入新节点名称"
              />
              <div className="mt-2 flex items-center justify-end gap-1.5">
                <button onClick={() => setPopup(null)} className="text-[11px] text-white/50 hover:text-white px-2 py-1">取消</button>
            ...
            <>
              ...
              <button
                ...
                className="w-full flex items-center gap-2 px-2.5 py-2 rounded-[8px] text-[12.5px] text-white/85 hover:text-white transition-colors"
                style={{ background: 'transparent' }}
                onMouseEnter={(e) => { e.currentTarget.style.background = 'rgba(255,255,255,0.12)'; }}
                onMouseLeave={(e) => { e.currentTarget.style.background = 'transparent'; }}
              >
                <Edit3 className="h-3.5 w-3.5 text-white/60" strokeWidth={1.6} />
                编辑节点名称
              </button>
              <button
                ...
                onMouseLeave={(e) => { e.currentTarget.style.background = 'transparent'; }}
              >
                <FileText className="h-3.5 w-3.5 text-white/60" strokeWidth={1.6} />
                添加内容
              </button>
            </>
          ) : (
            <>
              <div className="text-[12.5px] font-semibold text-white/90 truncate pr-6">{popupText}</div>
              {(() => {
                const n = allNodes.find((x) => x.id === popup.nodeId);
                const nt = n?.noteType || 'text';
                if (n?.note) {
                  return nt !== 'text' ? (
                    <pre className="mt-1.5 text-[11px] text-sky-100/80 leading-relaxed max-h-24 overflow-y-auto whitespace-pre-wrap rounded-md p-2"
                      style={{ fontFamily: 'var(--font-mono)', background: 'rgba(0,0,0,0.4)', border: '1px solid rgba(255,255,255,0.08)' }}>
                      {n.note}
                    </pre>
                  ) : (
                    <p className="mt-1.5 text-[11px] text-white/55 leading-relaxed max-h-16 overflow-y-auto whitespace-pre-wrap">{n.note}</p>
                  );
                }
                return <p className="mt-1.5 text-[11px] text-white/35">（无文档内容）</p>;
              })()}
      ...
      {/* 画布提示（左下角） */}
      <div className="absolute bottom-2.5 left-2.5 flex items-center gap-1.5 text-[10.5px] text-white/40 bg-black/40 rounded-[6px] px-2 py-1 pointer-events-none">
...
export default function LearningHub() {
  ...
  useEffect(() => {
    ...
    }).catch((e) => console.warn('load learning-hub failed:', e));
  }, []);
  ...
  const searchWeb = async (q = webQuery) => {
    const query = q.trim().slice(0, 200);
    if (!query) return;
    setWebLoading(true);
    setWebSearched(true);
    try {
      // 使用必应国内接口获取网页结果标题/摘要（尽力而为，失败则直接打开搜索页）
      const url = `https://cn.bing.com/search?q=${encodeURIComponent(query)}&format=rss`;
      const res = await fetch(url, { headers: { 'Accept': 'application/rss+xml' } }).catch(() => null);
      if (res && res.ok) {
        ...
        while ((m = itemRe.exec(xml)) && items.length < 8) {
          ...
          if (title && link) items.push({ title: title.replace(/<!\[CDATA\[|\]\]>/g, '').trim(), link, desc: desc.replace(/<!\[CDATA\[|\]\]>/g, '').slice(0, 180) });
        }
        if (items.length) { setWebResults(items); return; }
      }
      // RSS 失败则退化为直接打开
      setWebResults([]);
      openWebSearch(query);
  ...
  const renderTopBar = () => (
    <div className="flex items-end justify-between animate-slide-up">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="title-display text-[22px] tracking-tight">学习资料库</h1>
          <p className="text-[12.5px] text-white/50 mt-0.5">个人笔记 · 联网搜索 · draw.io 式思维导图</p>
        </div>
      </div>
    </div>
  );
    ...
    <div className="space-y-5 pb-10">
        ...
        <div className="space-y-4 animate-fade-in">
          ...
          <div className="glass p-3.5">
            <div className="flex items-center gap-2.5">
              <Globe className="h-4 w-4 text-sky-300 shrink-0" strokeWidth={1.6} />
              <input
                type="text"
                ...
                className="flex-1"
              />
              <button onClick={() => searchWeb()} disabled={webLoading || !webQuery.trim()} className="btn btn-default">
                {webLoading ? <span className="spinner" /> : <Search className="h-3.5 w-3.5" />}
                搜索
              </button>
            </div>
            {webSearched && webResults.length === 0 && !webLoading && (
              <p className="mt-2 text-[11.5px] text-white/40">已在浏览器打开搜索结果…</p>
            )}
            {webResults.length > 0 && (
              <div className="mt-3 space-y-2 max-h-80 overflow-y-auto">
                {webResults.map((r, i) => (
                  <div key={i} className="p-2.5 rounded-[8px] bg-white/[0.03] hover:bg-white/[0.06] transition-colors cursor-pointer border border-white/[0.05]" onClick={() => window.electronAPI?.openExternal?.(r.link)}>
                    <div className="text-[12.5px] font-medium text-sky-300 hover:text-sky-200 truncate">{r.title}</div>
                    {r.desc && <div className="text-[11px] text-white/45 mt-0.5 line-clamp-2">{r.desc}</div>}
                  </div>
                ))}
              </div>
            )}
          ...
          {filteredMaterials.length === 0 ? (
            <div className="glass py-16 text-center">
              <BookOpen className="h-8 w-8 mx-auto mb-3 opacity-40" strokeWidth={1.5} />
              <p className="text-[14px] font-medium text-white/70">{searchQuery || subjectFilter !== '全部' ? '没有匹配的资料' : '还没有学习资料'}</p>
              <p className="mt-1 text-[12px] text-white/40">点击"添加资料"开始记录</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {filteredMaterials.map((m, idx) => {
                ...
                return (
                  <div key={m.id} className="glass glass-hover p-4 animate-slide-up relative group cursor-pointer transition-all select-none" style={{ animationDelay: `${idx * 0.03}s` }} onDoubleClick={() => setDetailMaterial(m)} title="双击查看详情">
                    <div className="absolute top-3 right-3 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                      <button onClick={(e) => { e.stopPropagation(); openWebSearch(m.title); }} onDoubleClick={(e) => e.stopPropagation()} className="p-1.5 rounded-[6px] bg-black/40 text-white/50 hover:text-sky-300" title="网上搜索该主题">
                        <Globe className="h-3.5 w-3.5" />
                      </button>
                      <button onClick={(e) => { e.stopPropagation(); openEditModal(m); }} onDoubleClick={(e) => e.stopPropagation()} className="p-1.5 rounded-[6px] bg-black/40 text-white/50 hover:text-white" title="编辑">
                        <Edit3 className="h-3.5 w-3.5" />
                      </button>
                      <button onClick={(e) => { e.stopPropagation(); handleDeleteMaterial(m.id); }} onDoubleClick={(e) => e.stopPropagation()} className="p-1.5 rounded-[6px] bg-black/40 text-white/50 hover:text-red-300" title="删除">
                        <Trash2 className="h-3.5 w-3.5" />
                      </button>
                    </div>
                    <h3 className="text-[14px] font-semibold pr-20 truncate">{m.title}</h3>
                    <div className="mt-2 flex items-center gap-2 flex-wrap">
                      ...
                      {fmt !== 'text' && (
                        <span className="chip px-1.5 py-px text-[10px] font-semibold"
                          style={{
                            color: fmt === 'python' ? '#7dd3fc' : '#fcd34d',
                            background: fmt === 'python' ? 'rgba(56,189,248,0.12)' : 'rgba(251,191,36,0.12)',
                            borderColor: fmt === 'python' ? 'rgba(56,189,248,0.3)' : 'rgba(251,191,36,0.3)',
                          }}
                        >{fmt === 'python' ? '🐍 Python' : '🧮 MATLAB'}</span>
                      )}
                      {(m.tags || []).slice(0, 3).map((t) => <span key={t} className="text-[10.5px] text-white/40">#{t}</span>)}
                    </div>
                    {fmt !== 'text' ? (
                      <pre className="mt-2 text-[11px] text-sky-100/70 leading-relaxed rounded-md p-2.5 overflow-hidden whitespace-pre-wrap"
                        style={{ fontFamily: 'var(--font-mono)', background: 'rgba(0,0,0,0.4)', border: '1px solid rgba(255,255,255,0.06)', maxHeight: 72 }}>
                        {preview}
                      </pre>
                    ) : (
                      <p className="mt-2 text-[12px] text-white/60 leading-relaxed line-clamp-3 min-h-[3.5em]">{preview}</p>
                    )}
                    <div className="mt-3 pt-3 border-t border-white/[0.06] flex items-center justify-between">
                      <span className="text-[10.5px] text-white/40">{formatDate(m.updatedAt || m.createdAt)}</span>
                      <button onClick={(e) => { e.stopPropagation(); hasMm ? openMindmap(m.id) : generateMindmap(m); }} onDoubleClick={(e) => e.stopPropagation()} className="btn btn-default text-xs py-1.5 px-3">
      ...
      {showModal && (
        <div className="modal-overlay" onClick={() => { if (!formData.title.trim() && !formData.content.trim()) setShowModal(false); }}>
          <div className="modal-card mx-4 w-full max-w-lg p-5" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-[15px] font-semibold mb-4">{editingMaterial ? '编辑资料' : '添加资料'}</h2>
            <div className="space-y-3">
              <div>
                <label className="mb-1 block text-[11.5px] text-white/55 font-medium">标题</label>
              ...
              <div>
                <label className="mb-1 block text-[11.5px] text-white/55 font-medium">学科</label>
                <Dropdown
              ...
              <div>
                <label className="mb-1 block text-[11.5px] text-white/55 font-medium">格式</label>
                <div className="flex items-center gap-1.5">
                  {[
                    { v: 'text', label: '📝 文字', cls: 'text-white/80' },
                    { v: 'python', label: '🐍 Python', cls: 'text-sky-300' },
                    { v: 'matlab', label: '🧮 MATLAB', cls: 'text-amber-300' },
                  ].map((f) => (
                    <button
                      key={f.v}
                      type="button"
                      ...
                      style={{
                        background: formData.format === f.v ? 'rgba(255,255,255,0.14)' : 'rgba(255,255,255,0.04)',
                        border: `1px solid ${formData.format === f.v ? 'rgba(255,255,255,0.25)' : 'rgba(255,255,255,0.08)'}`,
                      }}
                    >
                      {f.label}
                    </button>
              ...
              <div>
                <label className="mb-1 block text-[11.5px] text-white/55 font-medium">内容</label>
                <textarea placeholder={formData.format === 'text' ? '学习笔记...' : `输入${formData.format === 'python' ? 'Python' : 'MATLAB'} 代码...`} value={formData.content} onChange={(e) => setFormData({ ...formData, content: e.target.value })} rows={8} className="w-full resize-none" style={formData.format !== 'text' ? { fontFamily: 'var(--font-mono)', background: 'rgba(0,0,0,0.35)', fontSize: '12.5px' } : undefined} spellCheck={false} />
              </div>
              <div>
                <label className="mb-1 block text-[11.5px] text-white/55 font-medium">标签（逗号分隔）</label>
      ...
      {detailMaterial && (
        <div className="modal-overlay" onClick={() => setDetailMaterial(null)}>
          <div className="modal-card mx-4 w-full max-w-3xl p-0 overflow-hidden flex flex-col" style={{ maxHeight: '88vh' }} onClick={(e) => e.stopPropagation()}>
            {/* 头部 */}
            <div className="px-5 pt-4 pb-3 flex items-start justify-between gap-3 border-b border-white/[0.06]">
              <div className="min-w-0">
                <h2 className="text-[17px] font-semibold tracking-tight break-words">{detailMaterial.title}</h2>
                <div className="mt-2 flex items-center gap-2 flex-wrap">
                  ...
                  {detailMaterial.format !== 'text' && (
                    <span className="chip px-1.5 py-px text-[10px] font-semibold"
                      style={{
                        color: detailMaterial.format === 'python' ? '#7dd3fc' : '#fcd34d',
                        background: detailMaterial.format === 'python' ? 'rgba(56,189,248,0.12)' : 'rgba(251,191,36,0.12)',
                        borderColor: detailMaterial.format === 'python' ? 'rgba(56,189,248,0.3)' : 'rgba(251,191,36,0.3)',
                      }}
                    >{detailMaterial.format === 'python' ? '🐍 Python' : '🧮 MATLAB'}</span>
                  )}
                  {(detailMaterial.tags || []).map((t) => <span key={t} className="text-[10.5px] text-white/40">#{t}</span>)}
                  <span className="text-[10.5px] text-white/35">{formatDate(detailMaterial.updatedAt || detailMaterial.createdAt)}</span>
                </div>
              </div>
              <div className="flex items-center gap-1.5 shrink-0">
                ...
                <button onClick={() => setDetailMaterial(null)} className="p-2 rounded-[8px] text-white/40 hover:text-white hover:bg-white/[0.08] transition-colors" title="关闭">
                  <X className="h-4 w-4" />
                </button>
              </div>
            </div>
            ...
            <div className="px-5 py-4 overflow-y-auto" style={{ fontFamily: 'var(--font-sans)' }}>
              {detailMaterial.format !== 'text' ? (
                <pre
                  ...
                  style={{ fontFamily: 'var(--font-mono)', background: 'rgba(0,0,0,0.45)', border: '1px solid rgba(255,255,255,0.08)', color: '#d8e6f2' }}
                >{detailMaterial.content}</pre>
              ) : (
                <p className="text-[13.5px] text-white/75 leading-relaxed whitespace-pre-wrap break-words select-text">{detailMaterial.content}</p>
              )}
            </div>
          </div>
        </div>
      ...
      {showNodeModal && (
        <div className="modal-overlay" onClick={() => setShowNodeModal(false)}>
          <div className="modal-card mx-4 w-full max-w-md p-5" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-[15px] font-semibold mb-3">{nodeModalMode === 'add' ? '添加子节点' : '编辑节点名称'}</h2>
      ...
      {noteModalOpen && (
        <div className="modal-overlay" onClick={() => setNoteModalOpen(false)}>
          <div className="modal-card mx-4 w-full max-w-2xl p-5" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-1">
              <h2 className="text-[15px] font-semibold">📄 节点内容</h2>
              <button onClick={() => setNoteModalOpen(false)} className="p-1.5 rounded-lg text-white/40 hover:text-white hover:bg-white/[0.06]">
                <X className="h-4 w-4" />
              </button>
            </div>
            <p className="text-[11.5px] text-white/45 mb-3">记录该节点的详细内容，支持文字与代码（Python / MATLAB）</p>
            ...
            <div className="mb-3 flex items-center gap-1.5">
              {[
                ...
                <button
                  ...
                  style={{
                    background: noteType === f.v ? 'rgba(255,255,255,0.14)' : 'rgba(255,255,255,0.04)',
                    border: `1px solid ${noteType === f.v ? 'rgba(255,255,255,0.25)' : 'rgba(255,255,255,0.08)'}`,
                  }}
                >
                  {f.label}
                </button>
            ...
            <textarea
              ...
              style={noteType !== 'text' ? { fontFamily: 'var(--font-mono)', background: 'rgba(0,0,0,0.35)', fontSize: '12.5px' } : undefined}
              autoFocus
              spellCheck={false}
            />
      ...
      {showExportModal && (
        <div className="modal-overlay" onClick={() => setShowExportModal(false)}>
          <div className="modal-card mx-4 w-full max-w-lg p-5" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-[15px] font-semibold mb-3">导出思维导图</h2>
            <pre className="w-full p-4 rounded-[8px] bg-white/[0.03] border border-white/[0.06] text-white/75 text-[12px] mono whitespace-pre-wrap max-h-80 overflow-auto mb-4" style={{ fontFamily: 'var(--font-mono)' }}>{exportText}</pre>
